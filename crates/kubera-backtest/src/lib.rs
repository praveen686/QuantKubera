...
// Parquet exports
use arrow::array::{Int64Array, Float64Array, StringBuilder, ArrayRef};
use arrow::datatypes::{DataType, Field, Schema};
use arrow::record_batch::RecordBatch;
use parquet::arrow::ArrowWriter;
use std::fs::File;

/// A single fill record for Parquet export.
#[derive(Debug, Clone)]
...
pub struct BacktestResults {
    ...
    pub returns: Vec<f64>,
    /// Timestamps for each equity curve point (nanoseconds since epoch).
    pub equity_timestamps: Vec<i64>,
    /// All fill records for Parquet export.
...
impl BacktestResults {
    ...
    /// Exports fills and equity curve to Parquet files.
    ///
    /// # Parameters
    /// * `output_dir` - Directory to write the Parquet files to.
    pub fn export_parquet(&self, output_dir: &Path) -> anyhow::Result<()> {
        std::fs::create_dir_all(output_dir)?;

        // Export fills.parquet
        if !self.fills.is_empty() {
            let ts_ns: Vec<i64> = self.fills.iter().map(|f| f.ts_ns).collect();
            let symbols: Vec<&str> = self.fills.iter().map(|f| f.symbol.as_str()).collect();
            let sides: Vec<&str> = self.fills.iter().map(|f| f.side.as_str()).collect();
            let quantities: Vec<f64> = self.fills.iter().map(|f| f.quantity).collect();
            let prices: Vec<f64> = self.fills.iter().map(|f| f.price).collect();
            let commissions: Vec<f64> = self.fills.iter().map(|f| f.commission).collect();
            ...
            let batch = RecordBatch::try_new(
                schema.clone(),
                vec![
                    Arc::new(Int64Array::from(ts_ns)) as ArrayRef,
            ...
            let file = File::create(output_dir.join("fills.parquet"))?;
            let mut writer = ArrowWriter::try_new(file, schema, None)?;
            writer.write(&batch)?;
            writer.close()?;
            info!("Exported fills.parquet with {} records", self.fills.len());
        }

        // Export equity_curve.parquet
        if !self.equity_curve.is_empty() {
            ...
            let batch = RecordBatch::try_new(
                schema.clone(),
                vec![
                    Arc::new(Int64Array::from(self.equity_timestamps.clone())) as ArrayRef,
                    Arc::new(Float64Array::from(self.equity_curve.clone())) as ArrayRef,
                ],
            )?;

            let file = File::create(output_dir.join("equity_curve.parquet"))?;
            let mut writer = ArrowWriter::try_new(file, schema, None)?;
            writer.write(&batch)?;
            writer.close()?;
            info!("Exported equity_curve.parquet with {} records", self.equity_curve.len());
        }
...
impl BacktestEngine {
    ...
    pub async fn run_range(
        ...
        while event_count < n_events {
            ...
            // Track timestamp from event
            self.current_ts_ns = event.exchange_time.timestamp_nanos_opt().unwrap_or(0);
            ...
            while let Ok(order_event) = order_rx.try_recv() {
                if let OrderPayload::Update { status, filled_quantity, avg_price, commission, .. } = order_event.payload {
                    if status == OrderStatus::Filled {
                        self.process_fill(&order_event.symbol, order_event.side, filled_quantity, avg_price, commission);
                    }
                }
            }
    ...
    fn update_equity(&mut self, current_price: f64) {
        ...
        self.results.equity_timestamps.push(self.current_ts_ns);
    ...
    fn process_fill(&mut self, symbol: &str, side: Side, quantity: f64, price: f64, commission: f64) {
        let pnl = self.portfolio.apply_fill(symbol, side, quantity, price, commission);

        // Record fill for Parquet export
        self.results.fills.push(FillRecord {
