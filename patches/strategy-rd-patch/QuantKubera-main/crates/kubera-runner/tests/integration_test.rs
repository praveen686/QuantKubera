...
use kubera_models::{MarketEvent, MarketPayload, Side, SignalEvent, OrderEvent, OrderPayload, FillEvent};
...
async fn test_full_trading_lifecycle() {
    ...
    let mut exchange = SimulatedExchange::new(bus.clone(), 5.0, None); // 5bps slippage
    let mut portfolio = Portfolio::new();
    
    let symbol = "BTCUSDT".to_string();
    
    // 2. Mock Market Event (Trade)
    let market_trade = MarketEvent {
