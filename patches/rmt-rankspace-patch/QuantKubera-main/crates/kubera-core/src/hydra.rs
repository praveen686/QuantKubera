...
impl Default for HydraConfig {
    fn default() -> Self {
        Self {
            ...
            rankspace_edge_scale_bps: 18.0, // No edge scaling by default
        }
    }
}
...
impl Expert for RankSpaceMeanRevExpert {
    ...
    fn update_market(&mut self, event: &MarketEvent, ctx: &HydraContext) {
        // Use mid if available; fallback to last trade.
    ...
    fn generate_intent(&self, _event: &MarketEvent, _ctx: &HydraContext) -> Option<ExpertIntent> {
        if self.signal.abs() < 0.05 {
            return None;
        }

        let side = if self.signal > 0.0 { Side::Buy } else { Side::Sell };
        let expected_edge_bps = self.edge_scale_bps * self.signal.abs();

        Some(ExpertIntent {
            side,
            confidence: self.signal.abs().clamp(0.0, 1.0),
            expected_edge_bps,
            reason: format!(
                "Rank-space MR (ts-rank) signal={:.3} edge={:.2}bps",
                self.signal, expected_edge_bps
            ),
        })
    }
}
